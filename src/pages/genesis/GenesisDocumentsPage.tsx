...
import { FileText, Upload, Trash2, Download, Building2 } from 'lucide-react'
...
function OrgDocuments({ orgId }: { orgId: string }) {
    ...
    return (
        <div className="space-y-4">
            {/* Upload bar */}
            <div className="flex flex-wrap items-center gap-3 rounded-lg border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900 px-4 py-3">
                ...
                <input ref={fileRef} type="file" className="hidden" id={`doc-upload-${orgId}`} onChange={(e) => void handleFile(e)} />
                <label htmlFor={`doc-upload-${orgId}`}
            ...
            {!docs || docs.length === 0 ? (
                <div className="rounded-lg border border-dashed border-gray-300 p-8 text-center">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">Nenhum documento cadastrado.</p>
                </div>
            ) : (
...
export function GenesisDocumentsPage() {
    const orgsQuery = useOrganizations()
    const [selectedOrgId, setSelectedOrgId] = useState<string>('')

    const orgs = useMemo(() => (orgsQuery.data ?? []).filter(o => o.status === 'active'), [orgsQuery.data])

    // Auto-select first org when loaded
    const effectiveOrgId = selectedOrgId || (orgs[0]?.id ?? '')

    if (orgsQuery.isLoading) return <SectionLoader />
    if (orgsQuery.error) return <ErrorMessage message="Erro ao carregar organizações" onRetry={() => void orgsQuery.refetch()} />
        ...
        <div className="space-y-6 p-6">
            <div>
                ...
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Gerencie documentos por organização cliente.
                </p>
            </div>
            ...
            <div className="flex items-center gap-3 rounded-lg border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900 px-4 py-3">
                ...
                {selectedOrg && (
                    <span className="text-xs text-gray-400">{selectedOrg.cnpj}</span>
                )}
            </div>
            ...
            {orgs.length === 0 && (
                <div className="rounded-lg border border-dashed border-gray-300 p-12 text-center">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">Nenhuma organização ativa encontrada.</p>
                </div>
            )}

            {effectiveOrgId && <OrgDocuments orgId={effectiveOrgId} />}
        </div>
    )
}
